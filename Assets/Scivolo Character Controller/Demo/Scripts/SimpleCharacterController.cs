...
namespace MenteBacata.ScivoloCharacterControllerDemo
{
    public class SimpleCharacterController : MonoBehaviour
    {
        ...
        public GameObject Cam, RespawnCubeParent;
        ...
        [SerializeField]
        AudioClip Walk1,Walk2,Walk3;


        private int CanSound;
        ...
        private void Update()
        {
            ...
            if (isGrounded) {
                ...
                if(isMoving ) {
                    ...
                    if (CanSound == 1 && !source.isPlaying) 
                        {
                            source.PlayOneShot(Walk1);
                        }

                    else if(CanSound == 2 && !source.isPlaying)
                    {
                        source.PlayOneShot(Walk2);
                    }

                    else if(CanSound == 3 && !source.isPlaying)
                    {
                        source.PlayOneShot(Walk3);
                    }
            ...
            if (Input.GetKeyDown(KeyCode.Escape) && !ui.TimeIsPaused)
            {
            ...
            else if(Input.GetKeyDown(KeyCode.Escape) && ui.TimeIsPaused)
            {
                ui.GoToSection();
                ui.UnPauseMenu();
